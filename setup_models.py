import os
"""
Setup script: generates synthetic bank marketing data and trains/saves all models.
Run this ONCE before launching the Streamlit app.
In production: replace with real bank-additional.csv and train in Colab.
"""
...
# ─────────────────────────────────────────────
# 1. GENERATE SYNTHETIC DATA (mimics UCI bank dataset)
# ─────────────────────────────────────────────
N = 4119  # bank-additional.csv size
...
poutcome_col = np.where(previous > 0, np.random.choice(['failure', 'success'], N, p=[0.65, 0.35]), 'nonexistent')
...
nr_employed = np.random.choice([4963.6, 5008.7, 5017.5, 5023.5, 5099.1, 5176.3, 5191.0, 5195.8, 5228.1], N)

# Create conversion probability based on real-world factors
logit = (
    -2.0
    + 0.01 * (age - 40)
    - 0.2 * (campaign > 5)
    + 1.5 * (poutcome_col == 'success')
    - 0.3 * (poutcome_col == 'failure')
    + 0.5 * (month_col.astype(str) in ['mar', 'oct', 'sep', 'dec'] if False else
             np.isin(month_col, ['mar', 'oct', 'sep', 'dec'])).astype(float)
...
df.to_csv(os.path.join(BASE, 'data', 'bank_data.csv'), index=False)
...
joblib.dump(lr, os.path.join(BASE, 'models', 'lr_model.pkl')

# Random Forest (lightweight: shallow trees)
...
joblib.dump(fi, os.path.join(BASE, 'models', 'feature_importance.pkl')
...
joblib.dump(kmeans, os.path.join(BASE, 'models', 'kmeans.pkl')
joblib.dump(seg_scaler, os.path.join(BASE, 'models', 'seg_scaler.pkl')
joblib.dump(seg_features, os.path.join(BASE, 'models', 'seg_features.pkl')
...
joblib.dump(cluster_profiles, os.path.join(BASE, 'models', 'cluster_profiles.pkl')
print(f"✓ K-Means clustering done: 4 segments")
print(cluster_profiles[['size', 'conversion_rate', 'label']])

# Save test results for metrics display
test_results = {
    'lr_auc': lr_auc,
    'rf_auc': rf_auc,
...
joblib.dump(test_results, os.path.join(BASE, 'models', 'test_results.pkl')

# Save df with clusters back
BASE = os.path.dirname(os.path.abspath(__file__))
os.makedirs(os.path.join(BASE, 'data'), exist_ok=True)
os.makedirs(os.path.join(BASE, 'models'), exist_ok=True)
df.to_csv(os.path.join(BASE, 'data', 'bank_data.csv'), index=False)
